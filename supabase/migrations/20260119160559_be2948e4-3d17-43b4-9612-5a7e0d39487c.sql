-- Update get_kitchen_items to include 'ready' status for delivery orders
CREATE OR REPLACE FUNCTION public.get_kitchen_items(_status_filter text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, table_order_id bigint, order_id bigint, product_id uuid, product_name text, quantity integer, observation text, unit_price numeric, status text, ordered_at timestamp with time zone, delivered_at timestamp with time zone, table_number integer, table_name text, waiter_name text, order_type text, customer_name text)
 LANGUAGE sql
 SECURITY DEFINER
