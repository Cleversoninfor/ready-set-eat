-- Kitchen feed RPC to avoid direct table exposure and bypass RLS safely
-- Returns only kitchen-needed fields (no phone/address)
  ...
  delivery_items as (
    select
      ...
      case
        ...
        when o.status = 'delivery' then 'ready'
        else 'pending'
      end as status,
    ...
    where o.status in ('pending', 'preparing')
      and (
        _status_filter is null
        or _status_filter in ('pending','preparing') and o.status = _status_filter
      )
  )
...
REVOKE ALL ON FUNCTION public.get_kitchen_items(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_kitchen_items(text) TO anon, authenticated;
